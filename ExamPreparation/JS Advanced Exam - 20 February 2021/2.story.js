class Story {
    ...
    comment(username, content, id) {
        ...
        let commentToReplayTo = this._comments.find(x => x.Id === id);
        let replyNextId = commentToReplayTo.Replies.length + 1;
        let replyId = Number(`${commentToReplayTo.Id}.${replyNextId}`)
        ...
        commentToReplayTo.Replies.push(reply);
    ...
    toString(sortingType) {
        ...
        let comments = this._comments.sort(sortingVersion[sortingType]);
        comments.forEach(x => x.Replies.sort(sortingVersion[sortingType]));
        let combinedStringArr = [];
        for (const comment of comments) {
            let commentsString = `-- ${comment.Id}. ${comment.Username}: ${comment.Content}`;
            let replaysString = comment.Replies.map(x => `--- ${x.Id}. ${x.Username}: ${x.Content}`)
                                               .join('\n');
            replaysString = comment.Replies.length > 0 ? `\n${replaysString}` : '';
            let combinedString = `${commentsString}${replaysString}`;
            combinedStringArr.push(combinedString);

        }
        let fullCommentsString = this._comments.length > 0 
        ? `\n${combinedStringArr.join('\n')}`
        : '';
